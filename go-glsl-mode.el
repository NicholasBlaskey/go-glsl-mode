...
;;     (when pos (message "Found pos keyword at %s %s" pos val))
;;     pos))
...
(defun go-add-sql-keyword-matcher ()
  ...
  (progn
    (font-lock-add-keywords
     nil
     '((match-regexes0 0 (nth 0 glsl-font-lock-keywords-1) t))
     'append)
    (font-lock-add-keywords
     nil
     '((match-regexes1 0 (nth 1 glsl-font-lock-keywords-1) t))
     'append)

;    (font-lock-add-keywords
;     nil
;     '((match-regexes3 0 (nth 3 glsl-font-lock-keywords-1) t))
;     'append)
;    )  
    )
  )

(defun match-regexes0 (end)
  (let (pos (case-fold-search t))
...
(defun match-regexes2 (end)
  (let (pos (case-fold-search t))
    (while (and (setq pos (re-search-forward (nth 0 (nth 2 glsl-font-lock-keywords-1)) end t))
                (null (nth 3 (syntax-ppss pos)))))
    (when pos (message "Found pos keyword at %s" pos))
    pos))
(defun match-regexes3 (end)
  (let (pos (case-fold-search t))
    (while (and (setq pos (re-search-forward (nth 0 (nth 3 glsl-font-lock-keywords-1)) end t))
                (null (nth 3 (syntax-ppss pos)))))
    (when pos (message "Found pos keyword at %s" pos))
    pos))
